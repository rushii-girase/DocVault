...
public class AuthController {
    ...
    public ResponseEntity<?> registerStudent(@Valid @RequestBody StudentSignupRequest signUpRequest) {
        ...
        System.out.println("----------------------------------------------------------------------");
        System.out.println("LOCAL VERIFICATION LINK: http://localhost:4200/verify-email?token=" + user.getVerificationToken());
        System.out.println("----------------------------------------------------------------------");
    ...
    public ResponseEntity<?> resetPassword(@RequestBody java.util.Map<String, String> payload) {
        ...
        if (user.getResetPasswordTokenExpiry() != null && user.getResetPasswordTokenExpiry().isBefore(LocalDateTime.now())) {
            return ResponseEntity.badRequest().body(new MessageResponse("Error: Password reset token expired."));
        }
        ...
        return ResponseEntity.ok(new MessageResponse("Password successfully updated. You can now login."));
    }
}
